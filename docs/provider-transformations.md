...
- map provider-reported or suite-estimated USD cost to `usage.costUsd`; omit it
  when unknown, use zero only for known-free runs, and keep provider-specific
  pricing details in `usage.metadata`
