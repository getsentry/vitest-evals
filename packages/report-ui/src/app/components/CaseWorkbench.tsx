...
export function CaseWorkbench({
  ...
  const showTools = allCases.some(
    (testCase) => (caseToolCallCount(testCase) ?? 0) > 0,
  );
        ...
        <tbody>
          {cases.map((testCase) => (
            <CaseRow
              ...
              showModel={columns.some((column) => column.id === "model")}
              showTools={columns.some((column) => column.id === "tools")}
              testCase={testCase}
              onSelectCase={onSelectCase}
            />
          ))}
