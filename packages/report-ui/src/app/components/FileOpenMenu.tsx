...
function CopyAbsPathButton({ file }: { file: string }) {
  ...
  return (
    <button
      className="block w-full px-3 py-1.5 text-left text-sm text-ink outline-none hover:bg-selected focus-visible:bg-selected"
      type="button"
      role="menuitem"
      onClick={async (event) => {
        event.stopPropagation();
    ...
    >
      {copied ? "Copied absolute path" : "Copy absolute path"}
    </button>
  );
}
