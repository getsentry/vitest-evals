...
description: Read transcript events and tool-call projections from a harness run.
editUrl: false
---
...
Use `toolCalls(result)` for the flattened tool history. It returns assistant
tool-call requests with `status`, `result`, and `error` filled from matching
`type: "tool_result"` transcript events that reference the call id. Calls
without a matching result have `status: "pending"`. The helpers also accept a
...
Read `result.session.events` directly when order across messages and tool
activity matters.
