...
judge output to your scoring code and records the underlying normalized judge
run on the score as `judgeRuns`. If one judge calls `runJudge()` more than once,
every run is retained. Matcher options win over a judge default, and a judge
...
Return a full `HarnessRun` from a custom judge harness when the provider exposes
usage. Stable token fields are included in terminal and GitHub report totals.
Keep provider-specific cost estimates under `usage.metadata`; reports preserve
that metadata but do not treat it as a normalized cost field.

```ts title="evals/judges/judgeHarness.ts"
const judgeHarness = createJudgeHarness({
  name: "rubric-model",
  async run(input) {
    const response = await callJudgeModel(input);
    return {
      ...
      usage: {
        ...
        metadata: { costUsd: response.costUsd },
      },
      errors: [],
    };
  },
