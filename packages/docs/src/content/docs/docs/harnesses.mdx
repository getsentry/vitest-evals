...
return the output, ordered transcript events, optional traces, and usage that
...
| `usage` | Provider, model, tokens, USD cost, tools, and retries. Omit unknown values. |

Set `usage.costUsd` to the provider-reported cost or the suite's estimate. Omit
it when cost is unknown; use `0` only when the run is known to be free.
