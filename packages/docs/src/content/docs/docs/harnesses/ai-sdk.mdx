...
for judges, replay, and reports. Tool-call transcript data comes from AI SDK
`steps`; custom `run` entrypoints that do not return steps use normalized
transcript events for local tool executions. Output-only custom runs still get
synthesized input/output messages; return a normalized `session` when evals
need exact transcript control beyond that.
