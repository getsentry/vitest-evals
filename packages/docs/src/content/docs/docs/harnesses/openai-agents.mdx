...
transcript data comes from OpenAI Agents run items such as `newItems` and
`output`; runtime tool wrappers enrich matching run items and provide transcript
events for custom entrypoints that return no provider items.
