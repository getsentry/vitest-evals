...
run/model spans to `result.traces` when provider data is available. Tool-call
transcript data comes from OpenAI Agents generated items: `newItems` first,
then `output` only when it contains recognizable SDK model-output items. SDK
`history` is used as a complete-history source when no generated items are
available. Runtime tool wrappers enrich matching run items and provide
transcript events for custom entrypoints that return no provider items.
