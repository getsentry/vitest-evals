...
ordered transcript event for the run. OpenAI/AI SDK-inspired `messages` are
also accepted at this boundary and are normalized into events, including
OpenAI-style `tool_calls` and AI SDK-style `tool-call`/`tool-result` content
parts. If another provider uses content blocks or item streams, adapt those into
`events` directly. Attach usage and artifacts when reports need them. It
...
session, usage, timings, artifacts, traces, and errors.
