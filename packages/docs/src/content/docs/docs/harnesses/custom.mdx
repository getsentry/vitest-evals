...
ordered transcript event for the run. Provider-style `messages` are also
accepted at this boundary and are normalized into events. Attach usage and
artifacts when reports need them. It automatically attaches a fallback run span
unless the harness returns its own traces.
