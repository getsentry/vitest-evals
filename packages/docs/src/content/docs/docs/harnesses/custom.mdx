...
Return `output` for assertions and judges; attach usage, ordered transcript
messages, and artifacts when reports need them. It automatically attaches a
fallback run span unless the harness returns its own traces.
